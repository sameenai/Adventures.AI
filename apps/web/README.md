...
| E2E | `npm run test:e2e` | 16 journeys × desktop + Pixel 7 mobile, incl. sign-in→plan→save→log-trip, an axe WCAG 2A/AA gate, and an image gate that fails on any 4xx image request a real page makes |
...
| Roadmap | `npm run roadmap:check` · part of `test:unit` (`roadmap.test.ts`) | the feature registry in `docs/roadmap/` stays true — a feature marked live must point at a route or page that exists, and `BOARD.md` must match the data it was generated from |
